...
<body style="margin: 0 !important;font-weight: bolder !important;font-family: 'Courier New', Courier, monospace;color: #fff">
    <div style="background-color: #0E1D3E;height: auto;">
        ...
        if(isset($_GET["qid"])) {
            ...
            if($res) {
                ...
                if($count == 0) {
                    ...
                    while ($row = mysqli_fetch_assoc($res)) { 
                        $answerArray[$i-1] = $row["answer"];
                        $random_array = ["op1","op2","op3","answer"];
                        $numbers = range(0, 3);
                        shuffle($numbers);
                        $randomNum = array_slice($numbers, 0, 4);
                        
                        echo "<script>console.log(".$row["answer"].")</script>";
                        echo $i.". ".$row["qs"]."<br>";
                        echo "<input type=\"radio\" value=\"".$row[$random_array[$randomNum[0]]]."\" name=\"ans".$i.$j."\">".$row[$random_array[$randomNum[0]]]."<br>";
                        echo "<input type=\"radio\" value=\"".$row[$random_array[$randomNum[1]]]."\" name=\"ans".$i.$j."\">".$row[$random_array[$randomNum[1]]]."<br>";               
                        echo "<input type=\"radio\" value=\"".$row[$random_array[$randomNum[2]]]."\" name=\"ans".$i.$j."\">".$row[$random_array[$randomNum[2]]]."<br>";               
                        echo "<input type=\"radio\" value=\"".$row[$random_array[$randomNum[3]]]."\" name=\"ans".$i.$j."\">".$row[$random_array[$randomNum[3]]]."<br><br>";
                        $i++;                            
                    }
            ...
            if(isset($_POST["submit"])) {
                $score = 0;
                for($i = 1; $i <= $count; $i++) {
                    if(isset($_POST["ans".$i.$j]) && $_POST["ans".$i.$j] == $answerArray[$i - 1]) {
                        $score++;
                    }  
                }
